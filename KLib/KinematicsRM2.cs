...
public class Manipulator6DoF
{
    ...
    public double[][] InverseGeometric(double[,] T_target, out bool[] flags, int verbose = 0)
    {
        ...
        foreach (var q1raw in new[] { q1a, q1b })
        {
            ...
            foreach (var q2 in q2Roots)
            {
                ...
                var T01 = RMLib.DHTransformRM(q1 + off[0], d[0], a[0], _mdh.Alpha[0]);
                var T12 = RMLib.DHTransformRM(q2 + off[1], d[1], a[1], _mdh.Alpha[1]);
                var T23 = RMLib.DHTransformRM(q3 + off[2], d[2], a[2], _mdh.Alpha[2]);
                var T03 = RMLib.MatMulRM4x4(RMLib.MatMulRM4x4(T01, T12), T23);
                var T36 = RMLib.MatMulRM4x4(RMLib.InvertHomogeneousRM(T03), T_target);
                ...
                foreach (var q5cand in new[] { Math.Acos(cos_q5), -Math.Acos(cos_q5) })
                {
                    ...
                    if (verbose >= 2)
                    {
                        ...
                        var T01d = RMLib.DHTransformRM(q1 + off[0], d[0], a[0], _mdh.Alpha[0]);
                        var T12d = RMLib.DHTransformRM(q2 + off[1], d[1], a[1], _mdh.Alpha[1]);
                        var T23d = RMLib.DHTransformRM(q3 + off[2], d[2], a[2], _mdh.Alpha[2]);
                        var T34d = RMLib.DHTransformRM(q4 + off[3], d[3], a[3], _mdh.Alpha[3]);
                        var T45d = RMLib.DHTransformRM(q5 + off[4], d[4], a[4], _mdh.Alpha[4]);
                        var T56d = RMLib.DHTransformRM(q6 + off[5], d[5], a[5], _mdh.Alpha[5]);

                        var T02d = RMLib.MatMulRM4x4(T01d, T12d);
                        var T03d = RMLib.MatMulRM4x4(T02d, T23d);
                        var T04d = RMLib.MatMulRM4x4(T03d, T34d);
                        var T05d = RMLib.MatMulRM4x4(T04d, T45d);
                        var T06d = RMLib.MatMulRM4x4(T05d, T56d);
        ...
        int verbose = 0)
    {
        ...
        for (var iter = 0; iter < maxIterations; iter++)
        {
            ...
            var cn = RMLib.CrossRM3x3(n_c, n_t);
            var co = RMLib.CrossRM3x3(o_c, o_t);
            var ca = RMLib.CrossRM3x3(a_c, a_t);
            ...
            var dq = RMLib.MatVecMulRM(JT, dX);
    ...
    private double[,] CalculateJacobian(double[] q)
    {
        ...
        for (var i = 0; i < 6; i++)
        {
            ...
            var Jv = RMLib.CrossRM3x3(zAxisPrev, p_diff);
    ...
    private double[] ComputeW2(double q1, double q2, double PwX, double PwY, double PwZ)
    {
        ...
        var T01 = RMLib.DHTransformRM(q1 + off[0], d[0], a[0], alpha[0]);
        var T12 = RMLib.DHTransformRM(q2 + off[1], d[1], a[1], alpha[1]);
        var T02 = RMLib.MatMulRM4x4(T01, T12);
