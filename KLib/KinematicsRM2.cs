    ...
    public static class ManipulatorFactory
    {
        ...
        private static readonly Dictionary<string, MDHParameters> _mdhTable = new(StringComparer.OrdinalIgnoreCase)
        {
            ["MiRobot"] = new MDHParameters(
                    ...
                    minDeg: [-179.9999, -179.9999, -89.9999, -179.9999, -179.9999, -359.9999],
                    maxDeg: [180.0,   90.0, 180.0,  180.0,   90.0,  360.0],
    ...
    public sealed class Manipulator6DoF
    {
        ...
        private double[] PoseErrorFull(double[,] T_cur, double[,] T_target)
        {
            ...
            else
            {
                ...
                w = new[] { axis[0] * angle, axis[1] * angle, axis[2] * angle };
            }
            ...
            int verbose = 0)
        {
            ...
            foreach (var q in rawSolutions)
            {
                ...
                var flipAxisAligned = angleDeg >= OrientationFlipRejectDeg && IsAxisAlignedFlip(Tcand, T_target);
